...
public class GraphPanel extends JPanel {
    ...
    public GraphPanel(Vector<Vector<Object>> revenueData) {
...
protected void paintComponent(Graphics g) {
    ...
    if (values.size() <= 1) {
        ...
        return; // Skip drawing the graph if not enough data
    }
    ...
    for (int i = 0; i < values.size(); i++) {
        ...
        if ((i % ((int) ((values.size() / 20.0)) + 1)) == 0) {
            ...
            try {
                Object month = revenueData.get(i).get(0); // Use month or label from column 0
                Object year = revenueData.get(i).get(1);
                String xLabel = month.toString()+"/"+year.toString()+"";
                FontMetrics metrics = g2.getFontMetrics();
    ...
    private List<Double> extractRevenueValues() {
        ...
        for (Vector<Object> row : revenueData) {
            try {
                Object val = row.get(2); // assuming revenue is in column 1
    ...
    public static void main(String[] args) {
            ...
            receiptBUS rb = new receiptBUS();
            Vector<Vector<Object>> mockData = rb.getRevenueStatisticsByMonth();
            GraphPanel mainPanel = new GraphPanel(mockData);
            mainPanel.setPreferredSize(new Dimension(800, 600));
            JFrame frame = new JFrame("Monthly Revenue Chart");
